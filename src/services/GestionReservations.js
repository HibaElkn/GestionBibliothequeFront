...
const GestionReservations = ({ onDeleteReservation, onAddReservation }) => {
    ...
    const handleStatutChange = async (id, newStatut, documentId) => {
        const updatedReservation = reservationsData.find(reservation => reservation.id === id);
    
        if (!updatedReservation) {
            setAlertMessage("Réservation introuvable.");
            return;
        }
        ...
        try {
            if (newStatut === 'ACCEPTED') {
                ...
                // // If saveEmprunt succeeds, update document availability
                // await documentService.updateDocumentAvailability(documentId, false);
            }
    
            // Update reservation status after successful operations
            ...
            console.error("Error handling reservation status change:", error);
    
            // Display error message
            if (error instanceof Error) {
                setAlertMessage(error.message);
            } else {
                setAlertMessage("Une erreur inconnue s'est produite.");
            }
        }
    };
